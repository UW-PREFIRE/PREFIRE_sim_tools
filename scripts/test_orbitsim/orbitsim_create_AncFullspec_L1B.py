...
def create_ancfullspec_L1B(ancillary_data_dir, anc_simtruth_in_fpath,
                           anc_st_sfcemis_in_fpath, output_dir,
                           product_fullver, provenance, allsky=True,
                           polewardlat=None, northonly=False,
                           write_rad_TIRSres=True):
    """
    Function to read in an ANC-SimTruth file and then produce full-spectrum
     modeled radiances using PCRTM (and then write them to a file).  Optionally,
     can write out the modeled radiances at TIRS resolution (based on the SRFs).
      ...
      #--- For TIRS-resolution spectra:
        ...
        for k in range(xtrack):
            ...
            if "SRFweighted_w" in SRFdata:
                weighted_w_xt[k,:] = SRFdata["SRFweighted_w"]
                idealized_w_xt[k,:] = SRFdata["idealized_w"]
            else:
                weighted_w_xt[k,:] = w
                idealized_w_xt[k,:] = w
...
if __name__ == "__main__":
    ...
    elif len(sys.argv)-1 == 5:  # Command-line argument(s) provided
    ...
    # if running as main
    create_ancfullspec_L1B(ancillary_data_dir, anc_simtruth_in_fpath,
                           anc_st_sfcemis_in_fpath, output_dir,
                           product_fullver, provenance, allsky=True,
                           write_rad_TIRSres=True)
